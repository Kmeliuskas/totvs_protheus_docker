...
var totvstec = {
	...
	connect: function(port, remoteType, callback) {
		this.internalWSPort = Number(port);
		...
		socket.onopen = function() {
			new QWebChannel(socket, function(channel) {
				...
				dialog.advplToJs.connect(function(codeType, codeContent) {
					totvstec.advplToJs(codeType, codeContent);
				});

				totvstec.jsToAdvpl = function(key, value) {
					window.dialog.jsToAdvpl(key, value);
				};
		...
		this.channel.port1.onmessage = function(event) {
			...
			switch (messageType) {
				case 'CONNECTED':
					...
					window.dialog.jsToAdvpl = totvstec.sendMessageChannel.bind(totvstec);

					totvstec.jsToAdvpl = totvstec.sendMessageChannel.bind(totvstec);
